...
def fetch_and_calculate(stock_code, start_date, end_date):
    df_total = None
...
def plot_stock_data(stock_code, df):
    ...
    ax1.xaxis.set_visible(False)  # 隱藏時間軸
    ...
    ax1.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, pos: f'{x:.2f}' if pos == 0 else ''))  # 只顯示最後的價格
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        futures = [executor.submit(fetch_and_calculate, stock_code, start_date, end_date) for stock_code in stock_symbols]
    ...
    for stock_code in df_top_growth['Stock']:
        ...
        if df_total is not None:
            ...
            image_path = plot_stock_data(stock_code, df_total)
